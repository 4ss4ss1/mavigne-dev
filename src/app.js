...
/* ⚠⚠⚠ CE BLOC DEVRAIT PARTIR, ET IL RESTE VOLONTAIREMENT. LIRE AVANT DE LE VIDER.
   Il porte le trace GPS des 46 parcelles du domaine de reference. A ce titre il
   releve de CONF-1 comme MEMBRES et PARCELLES — sauf que lui, PERSONNE NE LE
   REMPLACE. Les trois lecteurs ecrivent `KML_POLYGONS_DYNAMIC.length ? DYNAMIC :
   KML_DATA`, et pour marchand-grillot la cle Firestore `kml_polygons` est VIDE :
   ses contours n'ont jamais ete importes, ils vivent ici. L'ecran d'import de la
   console GT l'ecrit lui-meme en rouge (admin-gt.js, ~l.2529).
   ★ Le vider aujourd'hui = carte du domaine BLANCHE en production : plan de
   cuverie, Pilotage, tri des parcelles par proximite. En pleine vendange.
   ★★ L'ORDRE EST DONC : importer les 46 contours dans `kml_polygons` via la
   console GT, VERIFIER la carte, et SEULEMENT ENSUITE remplacer ce bloc par
   `const KML_DATA = [];`. Un lot separe, apres la vendange de preference.
   ⚠ Tant qu'il est la, la geometrie parcellaire du domaine reste dans le bundle
   et dans le depot. C'est un risque assume et date, pas un oubli. */
