...
/* ★★ CONF-3 (05/09/2026) — LES 46 CONTOURS SONT SORTIS DU CODE.
   Ce bloc portait le trace GPS des 46 parcelles du domaine de reference : nom
   de vigne et contour point par point, servis dans le bundle a TOUS les domaines
   et publies dans un depot ouvert. C'est un plan d'exploitation.
   ★ Il ne pouvait pas partir avec CONF-1 : pour ce domaine la cle Firestore
   `kml_polygons` etait VIDE — le vider alors aurait rendu la carte blanche. Les
   contours ont ete importes via la console GT (aller-retour verifie : 46 contours,
   301 points, zero ecart), la carte controlee a l'ecran, PUIS ce bloc vide.
   ⚠ Les trois lecteurs ecrivent `KML_POLYGONS_DYNAMIC.length ? DYNAMIC : KML_DATA`.
   Le repli rend maintenant un tableau vide, comme pour tout domaine avant son
   propre import. Si la carte se vidait, c'est que `kml_polygons` a disparu de la
   base — pas que ce bloc manque. Un garde plus bas (initMap) le signale. */
const KML_DATA = [];
...
function _visiteScenario(){
  ...
  //   maturite, mise en bouteille) lisent des donnees qu'AUCUN seed ne posait :
  //   ils se seraient ouverts VIDES, sans erreur — exactement le motif du
  //   Cuvier de juillet et du simulateur d'aout.
  //   ⚠️ TOUT est derive de window.PARCELLES. Ecrire des noms de parcelles en
  //   dur fabriquerait des analyses orphelines : _matSynth croise sur le nom
  //   REEL de la parcelle, pas sur celui qu'on aurait imagine.
  try{
    var _pA=(window.PARCELLES||[]).filter(function(p){ return p && p.nom && p.statut!=='Arrachee'; });
    var _pN=_pA.map(function(p){ return p.nom; });

    // a) Cepage — sans lui _parcCoul() rend '?' et les tuiles Rouges / Blancs
    //    de la synthese de maturite restent vides. On ne SURCHARGE jamais un
    //    cepage deja saisi : la demo complete la base du bac a sable, elle ne
    //    la reecrit pas.
    _pA.forEach(function(p,i){
    ...
    //    mais AUCUN cuMetal, donc la synthese cuivre du Pilotage ne trouvait
    //    zero ligne et l'onglet Conformite s'ouvrait sur un vide.
    //    Deux apports par an sur six campagnes passees, le second sur les deux
    //    premieres parcelles seulement : le cumul se lit en gradient, DEUX
    //    parcelles passent en « Vigilance », aucune en depassement. Un domaine
    //    ou les trois quarts des parcelles seraient en alerte ne serait pas une
    //    demonstration, ce serait un contre-argument.
    //    Les annees anterieures ne polluent pas le compteur de passages :
    //    _cfmPassages() filtre sur la saison consultee.
    var _cuY=now.getFullYear(), _cuT=[];
    for(var _k=6;_k>=1;_k--){
      var _yy=_cuY-_k;
      _cuT.push({produit:'Bouillie bordelaise RSR',type:'Cuivre',amm:'2020047',dar:21,drae:6,znt:5,
        dose:'2 kg/ha',cuMetal:2.2,operateur:'Marie',date:_yy+'-06-12',parcelles:_pN.slice(),note:'Pression mildiou'});
      if(_pN.length>2) _cuT.push({produit:'Bouillie bordelaise RSR',type:'Cuivre',amm:'2020047',dar:21,drae:6,znt:5,
        dose:'1,8 kg/ha',cuMetal:2.0,operateur:'Jean',date:_yy+'-07-04',parcelles:_pN.slice(0,2),note:'Reprise apres orage'});
    }
    (window.TRAITEMENTS||[]).forEach(function(t){
      if(t && t.type==='Cuivre' && t.cuMetal==null) t.cuMetal=0.3;
    });
    if(_cuT.length) _ap('traitements',(window.TRAITEMENTS||[]).concat(_cuT));

    // c) CONTROLE DE MATURITE — CAVE_VENDANGE.analyses etait un tableau VIDE.
    //    Trois releves par parcelle, a J-12 / J-6 / J-1 : deux mesures au moins
    //    sont necessaires pour que _matSynth calcule une VITESSE, et la vitesse
    //    est le vrai signal (deux parcelles au meme sucre ne se recoltent pas
    //    le meme jour si l'une monte deux fois plus vite).
    ...
    //    ⚠️ Object.assign() d'applyFbData est une copie de SURFACE : reposer
    //    _cvSeed.analyses remplace la reference dans le seed, pas celle deja
    //    posee sur window. On ecrit donc aux DEUX endroits — la ceinture 12bis
    //    ne fait que confirmer.
    if(_ana.length){ _cvSeed.analyses=_ana; if(window.CAVE_VENDANGE) window.CAVE_VENDANGE.analyses=_ana; }

    // d) UNE CUVEE EN BOUTEILLE — l'onglet « Mise en bouteille » du Chai
    //    n'affichait que « Aucune cuvee embouteillee » : l'archive par
    //    millesime, qui est la memoire du domaine, ne se voyait nulle part.
    if(window.CAVE_ELEVAGE && window.CAVE_ELEVAGE.cuvees && window.CAVE_ELEVAGE.cuvees.length){
      window.CAVE_ELEVAGE.cuvees.push({id:'cuv0',nom:'Gevrey-Chambertin VV',millesime:now.getFullYear()-3,
        tonneaux:[{annee:now.getFullYear()-3,nb:5}],statut:'embouteille',fml_terminee:true,sous_tire:true,
        last_ouillage:null,last_analyse:null,nb_bouteilles:1480,date_bouteille:_daysAgo(240)});
    }
  }catch(e){ if(window.logError)window.logError({level:'info',cat:'demo',msg:'seed cuivre/maturite/bouteille'}); }
...
.mvt-bk{font-size:var(--pt-nano,9.5px);font-weight:700;letter-spacing:1.2px;text-transform:uppercase;color:#C9A84C}
...
.mvt-eb{display:block;width:100%;box-sizing:border-box;font-family:inherit;font-weight:600;font-size:var(--pt-base,14px);border-radius:12px;padding:13px;cursor:pointer;border:1px solid #E8E0D2;background:#fff;color:#2A2521;margin-top:9px;text-decoration:none}
...
//    citer ailleurs — c'est ce qui empeche un querySelector de viser le vide.
// ⚠️ Chaque `id` doit avoir sa branche dans _mvtChapter() : un chapitre sans
...
function _mvtMenu(){
  ...
  // Rendu par familles. Une entree dont la famille est inconnue n'est PAS
  // perdue : elle rejoint la derniere famille plutot que de dispara\u00eetre en
  // silence — c'est le defaut qu'on ne verrait jamais a la relecture.
  var vus={}, grid='';
  _MVT_FAM.forEach(function(f){
  ...
  if(orph.length){ grid+='<div class="mvt-fam"><span class="mvt-fam-ic">\u2026</span><span>Et aussi</span><span class="mvt-fam-n">'+orph.length+'</span></div>'+orph.map(_mvtChBtn).join(''); }
...
function _mvtChapterClose(silent){
  ...
  // Les chapitres qui ouvrent un OVERLAY doivent le refermer en sortant :
  // sinon le menu se dessine dessous et le visiteur croit l'app bloquee.
  ['ovDocs','ovPlanFiche'].forEach(function(id){
    var ov=document.getElementById(id);
    if(ov && typeof window.closeOv==='function'){ try{ window.closeOv(null,id); }catch(e){ if(window.logError)window.logError({level:'info',cat:'visite',msg:'closeOv '+id}); } }
  });
  if(window._mvtOuvrierActive){
    try{ if(window.currentUser){ window.currentUser.roles=['admin','ouvrier','tractoriste']; } if(typeof currentUser!=='undefined'){ currentUser.roles=['admin','ouvrier','tractoriste']; } if(typeof applyRoles==='function')applyRoles(); }catch(e){ if(window.logError)window.logError({level:'info',cat:'visite',msg:'roles rendus'}); }
    window._mvtOuvrierActive=false;
  }
}
// Les onglets du Pilotage vises par les chapitres. Table SEPAREE plutot que
...
function _mvtGoCave(sec,after){
  ...
  setTimeout(function(){
    try{
      if(window.selectCaveSection) window.selectCaveSection(sec);
      if(after) after();
    }catch(e){ if(window.logError)window.logError({level:'info',cat:'demo',msg:'cave '+sec}); }
  },260);
}
function _mvtChapter(id){
  ...
  try{
    if(_MVT_PILTAB[id]){ _mvtGoPilTab(_MVT_PILTAB[id]); }
    else if(id==='carte'){ goTo('parcelles'); setTimeout(function(){ try{ if(window.switchPTab) window.switchPTab('carte'); }catch(e){ if(window.logError)window.logError({level:'info',cat:'demo',msg:'switchPTab carte'}); } },240); }
    else if(id==='parcelles'){ goTo('parcelles'); setTimeout(function(){ try{ if(window.switchPTab) window.switchPTab('liste'); }catch(e){ if(window.logError)window.logError({level:'info',cat:'demo',msg:'switchPTab liste'}); } },240); }
    else if(id==='journal'){ if(window.switchVigneOng) window.switchVigneOng('journal'); setTimeout(function(){ try{ if(window.renderJournal) window.renderJournal(); }catch(e){ if(window.logError)window.logError({level:'info',cat:'demo',msg:'renderJournal'}); } },200); }
    else if(id==='secteurs'){ goTo('home'); setTimeout(function(){ var el=document.getElementById('home-meteo-communes'); if(el){ try{ el.scrollIntoView({behavior:'smooth',block:'center'}); }catch(e){ if(window.logError)window.logError({level:'info',cat:'demo',msg:'scroll secteurs'}); } } },340); }
    else if(id==='cave'){ _mvtGoCave('elevage',function(){ if(window.switchCaveOng) window.switchCaveOng('cuv'); }); }
    ...
    else if(id==='tracteur'){ goTo('tracteur'); setTimeout(function(){ try{ if(window.switchTracOnglet) window.switchTracOnglet('sessions'); }catch(e){ if(window.logError)window.logError({level:'info',cat:'demo',msg:'trac sessions'}); } },220); }
    else if(id==='entretien'){ goTo('tracteur'); setTimeout(function(){ try{ if(window.switchTracOnglet) window.switchTracOnglet('entretiens'); }catch(e){ if(window.logError)window.logError({level:'info',cat:'demo',msg:'trac entretiens'}); } },220); }
    ...
    else if(window.logError){ window.logError({level:'info',cat:'demo',msg:'chapitre sans branche : '+id}); }
  }catch(e){ if(window.logError)window.logError({level:'info',cat:'demo',msg:'chapitre '+id+' : '+(e&&e.message)}); }
...
function _mvTrialBanner(){
  ...
  bar.innerHTML='<div class="mvtb-in" style="color:'+col.fg+'">'
    ...
    +'<span class="mvtb-txt"><b>'+label+'</b>'
      +(sous?('<span style="display:block;font-size:var(--pt-micro,11px);font-weight:500;opacity:.85;line-height:1.3;margin-top:1px">'+sous+'</span>'):'')
    +'</span>'
...
function _dmrInjectCss(){
  ...
  s.textContent = ''
    ...
    + '.dmr-mk{flex:0 0 20px;width:20px;height:20px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:var(--pt-micro,11px);margin-top:1px}'
...
function _relNivBadgesHtml(p) {
  ...
  var html = Array.from({length:planNb},function(_,i){return i+1;}).map(function(l) {
    ...
    return '<span style="display:inline-flex;align-items:center;padding:2px 7px;border-radius:5px;font-size:var(--pt-micro,11px);font-weight:700;background:'+bg+';color:'+col+';border:1px solid '+brd+'">'+lbl+' N'+l+'</span>';
  }).join('');
  if(st.ov != null && st.ov < planGlobal) {
    html += ' <span style="font-size:var(--pt-micro,11px);color:var(--ink-info,#4A9FC8)"></span>';
  }
  return html;
}
...
function _passBadgesHtml(p, nomTache) {
  ...
  for (var i = 1; i <= planNb; i++) {
    ...
    html += '<span style="display:inline-flex;align-items:center;padding:2px 7px;border-radius:5px;font-size:var(--pt-micro,11px);font-weight:700;'
...
function _pvRenderProxBar(){
  ...
  if(!el)return;
  if(_pProxLoading){
    el.innerHTML='<button disabled style="width:100%;display:flex;align-items:center;justify-content:center;gap:8px;min-height:44px;border:none;border-radius:12px;background:var(--gris-clair);color:var(--texte-doux);font-size:var(--pt-base,14px);font-weight:600;font-family:inherit">Localisation…</button>';
    return;
  }
  if(_pProxPos){
...
function renderParcelles(){
  ...
  cont.innerHTML=data.map(p=>{
    ...
    const _proxPill=(_proxD!=null)?`<span style="display:inline-flex;align-items:center;gap:3px;background:${_proxHere?'#C0845A':'var(--gris-clair)'};color:${_proxHere?'#fff':'var(--texte-doux)'};border-radius:20px;padding:2px 8px;font-size:var(--pt-micro,11px);font-weight:700;margin-left:6px">${_proxHere?'ici':''+_geoFmtDist(_proxD)}</span>`:'';
...
function _renderNiveauxModal(){
  ...
  if(_nivOv!==null&&_nivOv!==planGlobal){
    html+='<div style="margin-top:8px;padding:8px 12px;border-radius:7px;background:rgba(74,159,200,0.08);border:1px solid rgba(74,159,200,0.2);color:var(--acier-med);font-size:var(--pt-micro,11px)">&#9881; Override activ\u00e9 — cette parcelle diff\u00e8re du plan saison</div>';
  }
  html+='</div>';
  body.innerHTML=html;
}
...
function initMap(){
  ...
  /* ★ CONF-3 — UNE CARTE VIDE NE DOIT PAS SE TAIRE. Depuis que KML_DATA est vide,
     la seule source de contours est Firestore. Un domaine qui a des parcelles mais
     aucun contour affichait une carte nue, sans un mot : impossible a distinguer
     d'un domaine qui n'a jamais importe son KML. On le journalise — le rapport
     d'erreurs le fera remonter au lieu de laisser chercher. */
  if(!_kmlSource.length && (PARCELLES||[]).length && window.logError){
    window.logError({level:'info', cat:'carte',
      msg:'Carte sans contour : aucun polygone en base pour ce domaine',
      detail:(PARCELLES||[]).length+' parcelle(s), 0 contour'});
  }
...
function lancerExportEntretienPDF(){
  ...
  var _entCss = ''
    ...
    + '.fiche-date{font-weight:bold;font-size:var(--pt-micro,11px);}'
    ...
    + '.rep-motif{font-weight:bold;font-size:var(--pt-micro,11px);margin-bottom:4px;}'
...
.cov-meta{font-size:var(--pt-micro,11px);color:rgba(255,255,255,.42);margin-top:14px;display:flex;flex-wrap:wrap;gap:6px 16px;align-items:center}
...
.cov-state{display:inline-flex;align-items:center;gap:6px;background:rgba(78,125,52,.18);border:1px solid rgba(139,196,116,.35);
  color:var(--vert-clair);font-size:var(--pt-nano,9.5px);font-weight:700;text-transform:uppercase;letter-spacing:1px;padding:3px 9px;border-radius:20px}
...
.sec-note{font-size:var(--pt-lbl,10.5px);color:var(--texte-doux);font-style:italic;line-height:1.45;margin:-4px 0 12px}
/* ── Synthèse boxes ── */
...
/* ── Tables ── */
table{width:100%;border-collapse:collapse;font-size:var(--pt-micro,11px)}
...
.tot td{border-top:2px solid var(--cave);font-weight:800;background:#F3EEE4!important;font-size:var(--pt-micro,11px)}
.pnom{font-weight:700}
.muted{color:var(--texte-doux)}
.emptyrow td{color:var(--texte-doux);font-style:italic;padding:10px 9px}
.chip-d{display:inline-block;background:var(--ligne-2);border-radius:5px;padding:2px 7px;font-size:var(--pt-nano,9.5px);font-weight:600;color:#5c5348}
...
.pct{font-weight:800;font-size:var(--pt-micro,11px)}
/* ── Tags ── */
...
.inc-r .big{font-size:var(--pt-micro,11px);display:block}
...
.cu-band .txt{font-size:var(--pt-micro,11px);color:#6a5638}
...
.cu-row .nm{width:150px;font-size:var(--pt-micro,11px);font-weight:700;flex-shrink:0}
...
.cu-row .rt{width:96px;text-align:right;font-size:var(--pt-lbl,10.5px);font-weight:800;flex-shrink:0}
/* ── Heures / ETP ── */
...
.etp-cell .v{font-size:var(--pt-md,20px);font-weight:800;color:var(--cave)}
...
.sig{flex:1;border-top:1px solid #D8D1C5;padding-top:8px;font-size:var(--pt-lbl,10.5px);color:var(--texte-doux);text-align:center;font-weight:600}
...
function openRapportSaison(){
  ...
  list.innerHTML=arr.map(function(s){
    ...
    return '<div onclick="closeOv(null,\'ovRapportSaison\');exportRapportSaison(\''+nomEsc+'\')" '
      ...
      +'<div style="width:40px;height:40px;border-radius:11px;background:linear-gradient(135deg,#7A1020,#8A5A38);display:flex;align-items:center;justify-content:center;font-size:var(--pt-md,20px);flex-shrink:0">\uD83D\uDCC4</div>'
      ...
      +(sub?'<div style="font-size:var(--pt-micro,11px);color:var(--texte-doux,#8a8a7a);margin-top:2px">'+sub+'</div>':'')+'</div>'
...
function exportRapportSaison(seasonNom){
  ...
  try{
    ...
    if(_rich){
      ...
      etpBand=(tagf?'<div style="font-size:var(--pt-micro,11px);color:var(--texte-doux);font-weight:700;margin-bottom:8px">'+tagf+'</div>':'')
        +'<div class="etp-band">'
