...
var _EMH={nom:'',t:'',d:'',type:'',fin:'',grille:'',v:0};
...
function _emhForm(nom,t){
  ...
  if(t==='embauche'){
    h+='<div class="emh-s" style="margin-bottom:6px">Un nouveau contrat. Le pr\u00e9c\u00e9dent, s\u2019il existe, sera archiv\u00e9\u00a0\u2014 il reste lisible et imprimable.</div>'
     +'<div class="fl">Date de d\u00e9but</div><input type="date" class="emh-in" id="emh-d" oninput="_EMH.d=this.value;_emhEff()">'
     ...
     +'<input type="date" class="emh-in" id="emh-f" oninput="_EMH.fin=this.value;_emhEff()">';
  } else if(t==='renouvellement'){
    h+='<div class="emh-s" style="margin-bottom:6px">Le contrat continue. Seule sa date de fin bouge.</div>'
     +'<div class="fl">Date de signature</div><input type="date" class="emh-in" id="emh-d" value="'+_escAttr(auj)+'" oninput="_EMH.d=this.value;_emhEff()">'
     +'<div class="fl" style="margin-top:10px">Nouvelle date de fin</div><input type="date" class="emh-in" id="emh-f" oninput="_EMH.fin=this.value;_emhEff()">';
  } else if(t==='fin'){
    h+='<div class="emh-s" style="margin-bottom:6px">La fin r\u00e9elle. Elle prime sur la fin pr\u00e9vue, dans les deux sens.</div>'
     +'<div class="fl">Dernier jour travaill\u00e9</div><input type="date" class="emh-in" id="emh-d" value="'+_escAttr(auj)+'" oninput="_EMH.d=this.value;_emhEff()">';
  } else {
    h+='<div class="emh-s" style="margin-bottom:6px">Une augmentation ouvre une p\u00e9riode et laisse le pass\u00e9 intact.</div>'
     ...
     +'<input type="number" step="0.10" min="0" inputmode="decimal" class="emh-in" id="emh-v" value="'+_escAttr(String(_EMH.v||''))+'" oninput="_EMH.v=parseFloat(this.value)||0;_emhEff()">'
     +'<div class="fl" style="margin-top:10px">\u00c0 partir du</div><input type="date" class="emh-in" id="emh-d" value="'+_escAttr(auj)+'" oninput="_EMH.d=this.value;_emhEff()">';
  }
...
function _emhTyPick(){
  ...
  el.innerHTML=['G\u00e9rant','CDI','CDD','TESA','Apprenti','Saisonnier','Extra'].map(function(t){
    return '<div class="pchk'+(_EMH.type===t?' sel vert':'')+'" onclick="_EMH.type=\''+t+'\';_emhTyPick();_emhEff()">'+t+'</div>';
  }).join('');
}
function _emhGrPick(){
  var el=document.getElementById('emh-gr'); if(!el) return;
  el.innerHTML=_emhGrilles().map(function(g){
    return '<div class="pchk'+(_EMH.grille===g?' sel vert':'')+'" onclick="_EMH.grille=\''+_escAttr(g)+'\';_emhGrPick();_emhEff()">'+_escHtml(g)+'</div>';
  }).join('');
}
function _emhEvt(){
  var E=_EMH;
...
function _emhEff(){
  ...
  if(_EMH.t==='taux'){
    ...
    box.innerHTML='\u00c0 partir du <b>'+_emhFmt(_EMH.d)+'</b>, le co\u00fbt du travail passe \u00e0 <b>'
      +String(Math.round(_EMH.v*100)/100).replace('.',',')+'\u00a0\u20AC/h</b>. Les heures d\u2019avant gardent leur taux.';
    return;
  }
  var m=(window.MEMBRES||[]).find(function(x){return x.nom===_EMH.nom;});
  ...
  } else if(_EMH.t==='renouvellement'){
    box.className='emh-eff good';
    box.innerHTML='<b>Toujours un seul contrat</b>, donc un seul compteur. Il court d\u00e9sormais jusqu\u2019au <b>'
      +_emhFmt(_EMH.fin)+'</b>. L\u2019ancienne date de fin reste dans l\u2019historique.';
  } else if(_EMH.t==='fin'){
    box.className='emh-eff';
    box.innerHTML='Le contrat s\u2019arr\u00eate le <b>'+_emhFmt(_EMH.d)+'</b>. Son compteur est sold\u00e9. '
...
function _emhOk(){
  ...
  var nom=_EMH.nom, m=(window.MEMBRES||[]).find(function(x){return x.nom===nom;});
  if(!m) return;
  if(e.t==='taux'){
