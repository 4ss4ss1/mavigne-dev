...
//   rétablie ; touch-action:manipulation ; zones d'appui des puces ; décalage sous le bandeau de la démo. APP 7.58 → 7.59.
// v8.27 (23/09/2026) — TOUR-1 : affichage — textes illisibles corrigés (session tracteur en cours et carte « Travaux
//   mécaniques » : couleur perdue depuis TYPO-1 ; date de la barre méta ; week-ends et année du Planning ; Pilotage et
//   Réglages en sombre : --cave pris comme couleur de texte ; --bordeaux devient un jeton des deux thèmes) ; ligne de cuve du
//   Cuvier ré-empilée (spans sans display:block) ; champs blancs en dur → --bg-card (tournée, maturités, bouteilles,
//   relevés, porte CGU) ; panneau « Ce qu'il manque » au-dessus du dock. APP 7.57 → 7.58.
// v8.26 (23/09/2026) — CHAMP-1 + CHAMP-2 : les heures « dans les rangs » (_planChampPersRange, planning.js — formation
//   et événement familial à 0, absence partielle amputée de ses heures) remplacent le travail effectif de la loi dans
//   Pilotage › Décider (équipe du jour de la tournée et de « Qui fait quoi » ; la liste dit « en formation », « en arrêt »)
//   ET dans la cadence (cadence d'équipe, écart de cadence face au barème, période en cours et campagne d'avant). Paie,
//   compteur, taux horaire et exercice inchangés. APP 7.55 → 7.57. (v8.25 / 7.56 jamais poussées : remplacées.)
...
//   glisse de plus de 16 px, second appui à moins de 0,6 s. Décocher une parcelle demande confirmation. Aide : le point
//   « Changer d'année » quitte la fiche Tracteur pour celle du Planning. APP 7.52 → 7.53.
...
//   « Maintenu » devient « Aucune retenue ». « deja majorees » devient « a payer sans majoration (elle est deja dans la
//   recup) ». Heures restantes rangees de la meme facon (papier, ecran). APP 7.46 -> 7.47.
...
//   tout premier install (garde sur navigator.serviceWorker.controller). APP 7.44 -> 7.45.
...
//   salarie se reprennent sur la recup et les heures sup avant tout paiement, celles du domaine aussi puis vont au compteur
//   des heures a rattraper (jamais de retenue) ; le releve compte tout le mois (jours a venir aux heures du planning, plus
//   de provisoire) ; plus d'absence sans motif (injustifiee depuis septembre 2026). APP 7.39 -> 7.40.
// v8.04 (19/09/2026) — FUT-CAP-2 : une cuvee remise en elevage depuis sa fiche reprend ses futs au parc (ou refuse s'ils
//   n'y sont plus) ; l'ouillage a prevoir compte chaque fut a sa contenance. APP 7.38 -> 7.39.
// v8.03 (19/09/2026) — FUT-CAP + CUV-14 : chaque lot de futs a sa contenance (Reserve, decuvage, Chai, part des anges,
//   bilan de campagne, retrait) ; le volume decuve se saisit au decuvage et se corrige ensuite — c'est lui qui fait le
//   rendement ; « Modifier la cuvee » garde tonnelier, reference et contenance. APP 7.37 -> 7.38.
...
//   APP 7.33 -> 7.34.
// v7.97 (18/09/2026) — SEM-2 : LE RELEVE D'HEURES v3. Absences par cause, semaine entiere, releve provisoire en cours de
//   mois, compteur depuis le solde d'avant, compte des heures a rattraper, mentions et alertes. APP 7.32 -> 7.33.
// v7.96 (17/09/2026) — SEM-1 : LES HEURES SUP SE COMPTENT A LA SEMAINE. Les heures en plus rattrapent d'abord les heures
//   manquees de la meme semaine ; huit heures sup a 25 %, les suivantes a 50 % (le rang, plus la 44e heure travaillee) ;
//   une semaine appartient au mois ou elle finit ; un jour sans saisie vaut les heures du modele. APP 7.31 -> 7.32.
...
// v7.93 (17/09/2026) — FICHE-3 : UN NOMBRE D'HEURES PAR TAUX, ET CE QU'IL RESTE A PAYER. Demande de paiement :
//   le cadre donne 25 %, 50 %, dimanche, ferie (meme a zero) ; l'onglet Compteur montre les heures sup restantes a
//   payer, tranche par tranche du compteur redevenue heures brutes, avec le decompte et l'equivalent en recup.
...
//   vie) et a rendre (loue, fin de contrat). Achats : 4e source, loyer en lecture
//   seule, ecran borne a l'exercice. Trois etats absent/zero/montant retablis sur
//   le prix d'un achat Reserve et sur le loyer. Fusion silencieuse des lots
//   retiree : deux factures = deux lots, avec date de facture. PORTES : la chaine
//   locale (check/prebuild) couvre enfin les 14 invocations que seul le CI lancait,
//   et mv-harnais-portes.mjs empeche la prochaine derive. APP 7.02 -> 7.03.
...
//   (horsBornes) ; Presences hors bureau ; masse salariale AVEC le bureau (4e
//   argument de _mvEnContratSurPeriode) ; _ecoRate pondere heures x effectif ;
//   rythme borne au debut de periode ; photo Travaux au prorata. L'exercice est
//   COUPE AU JOUR : engage / prevu (grille) / a la cloture, N-1 aux memes jours
//   (_pexData 3e argument). Une seule liste « a completer » : _pecZeros relit
//   _pilDiag (GNR et phyto y entrent, cible phyto). APP 7.00 -> 7.01.
// v7.59 (09/09/2026) — CAVE-6 : L'ORDRE DU CUVIER, LE FILTRE DU CHAI, LES
//   POLICES. Les trois onglets du Cuvier suivent la vendange (Maturites,
//   Recoltes, Cuves) ; cles, handlers et onglet d'arrivee inchanges. Le filtre
//   millesime quitte la liste des cuvees pour #mvc-milbar, sous la barre
//   d'onglets : les TROIS vues du Chai s'y accrochent par _caveDansFiltre, le
//   journal via _rmMilCuvees (celle du registre imprime, pas une seconde
//   definition) ; une operation non rattachable sort du filtre ET est comptee a
//   l'ecran. 415 tailles de texte en dur passent aux onze pas --pt-* avec un
//   plancher a 9,5 px ; les 68 des feuilles de documents restent en dur (un
//   document ne charge pas styles.css, §86). APP 7.00.
...
//   depot. Ces numeros ont donc pu partir en ligne. Regle du doute SW :
//   reutiliser un numero deja deploye figerait l'ancien index.html pour
//   toujours chez ceux qui l'ont recu.
//   ★ LE VRAI DEFAUT DU LOT N'ETAIT PAS LE NOMBRE D'EMOJIS, C'ETAIT L'ANGLE MORT :
//   le cliquet ne lisait que src/*.js. index.html — les modales, les titres de
//   section, les puces de filtre, tout l'ecran Reglages, la surface que le
//   client voit le PLUS — portait 257 pictogrammes que RIEN ne comptait. Le
//   harnais sortait 27 verts depuis le premier jour de DS-1, en toute bonne foi.
//   257 -> 1 (le seul restant est `data-emoji="..."`, une VALEUR en base).
//   ★ Sprite 59 -> 94 symboles, Lucide EPINGLE en 1.31.0 : le paquet est passe
//   a 1.33 entre-temps et regenerer aurait modifie les 59 formes deja validees
//   a l'oeil. Verifie : regeneration identique A L'OCTET sur les 59 anciennes.
//   ⚠️⚠️ TROISIEME FOIS QUE LE COMPTEUR MENT, apres les echappements (§50) et le
//   selecteur U+FE0F (§45a) : la classe de caracteres avait un TROU. U+2139 (le
//   « i » d'information) tombait entre deux plages, 13 occurrences rendues et
//   invisibles dans SEPT fichiers. Le hasard des bornes n'est pas une definition.
//   ★ La classe est desormais l'UNION des plages historiques et de
//   \\p{Extended_Pictographic}. Prendre la propriete Unicode SEULE aurait fait
//   tomber le compte de 1341 a ~700 en silence : « ✓ », 113 occurrences, le
//   glyphe le plus frequent de l'application, n'en fait pas partie. Un cliquet
//   qui degringole tout seul n'est pas un progres, c'est une mesure qui a
//   change de sujet.
//   ★ La boucle de hausse itere sur les CLES du comptage, plus sur la liste
//   MODULES : c'est exactement l'ecriture qui avait laisse index.html dehors.
//   ★ 252 substitutions posees de la FIN vers le DEBUT, chaque ancre verifiee
//   avant ecriture. Preuve contre le piege de §45e (le nettoyage qui mange les
//   espaces legitimes) : 40 922 mots IDENTIQUES avant/apres, mesures.
//   ★ 17 contre-epreuves, dont deux neuves : un emoji repose dans index.html,
//   et un glyphe hors des plages historiques. La seconde a ete elle-meme
//   contre-eprouvee — classe retrecie, elle RESTE VERTE, donc elle mord.
//   ⚠️ Trois glyphes restent, NOMMES avec leur raison : © (signe legal),
//   ◼ et ▭ (ils DESSINENT les deux tailles de carte dans le bandeau
//   d'edition ; une icone a leur place dirait moins que le glyphe).
...
//   ★★ CE N'EST PAS UN SECOND FORMULAIRE, c'est un ecran de VALORISATION : le
//   FAIT reste dans son module, seul le PRIX se met ici — et il retourne DANS
//   L'OBJET (achats[].prix, futs[].prix, reparateur_hist[].eur), jamais dans une
//   table de prix a part qui laisserait des orphelins.
//   ★ « Achat » OUVRE le formulaire du module proprietaire (_rsvOpenAchat,
//   _rsvOpenFut), il ne le recopie pas : un intrant phyto demande le catalogue
//   E-Phy, et deux formulaires pour la meme chose divergent au premier lot.
//   ⚠⚠ TROIS ETATS, JAMAIS DEUX : absent = a chiffrer, 0 = SANS FRAIS (garantie,
//   geste commercial), >0 = chiffre. Un test truthy confondrait les deux premiers
//   et laisserait une reparation sous garantie « a chiffrer » a vie, en faisant
//   mentir le compteur d'en-tete tous les mois.
//   ★ La source des reparations est REPARATEUR_HIST, PAS les fiches d'entretien :
//   le cycle « emmene chez le reparateur -> rentre » existait deja et EST le
//   signal d'une intervention payante. Une checklist quotidienne (plein, huile,
//   pression) n'est pas un achat — la faire remonter aurait noye l'ecran.
//   Deux champs ajoutes : `four` sur le cycle reparateur (saisi au DEPART, on
//   sait ou on emmene la machine) et `prix` sur le lot de futs. Aucun montant
//   n'est demande au tractoriste : il rend les cles, il n'a pas la facture.
//   ⚠ Revient sur le lot precedent, NON DEPLOYE : l'onglet « Depenses » de La
//   Reserve et la cle INTRANTS.depenses sont retires. Une revision de tracteur
//   n'a rien a faire dans La Reserve — la depense se note la ou le fait se note.
//   Le journal des nouveautes et cette entree sont REECRITS plutot qu'empiles :
//   empiler raconterait aux clients une fonctionnalite qu'ils n'ont jamais vue.
//   ⚠ C23 a attrape la vraie casse du retrait : `_eur2` vivait dans le bloc
//   supprime alors que le prix moyen l'appelle encore — ecran Intrants blanc au
//   premier rendu, invisible pour node --check comme pour ESLint.
//   ★ Economie > Exercice repartit les consommables par ATELIER (vigne, cave,
//   tracteur, non affecte) en plus de leur nature ; la somme des ateliers vaut
//   exactement achats + GNR + reparations, et la ligne de controle l'AFFICHE.
//   Les salaires n'y sont pas : une entree de planning porte des heures, jamais
//   une activite.
//   ⚠⚠ DEFAUT CORRIGE, ACTIF DEPUIS LE PREMIER JOUR : produits[].prixU n'avait
//   AUCUNE interface d'ecriture. Le cout phyto par parcelle etait a ZERO chez
//   tous les domaines reels. Il derive maintenant d'un PMP sur achats[].prix ;
//   les lignes sans prix sont ECARTEES du quotient (11,8 % d'ecart mesure).
//   ★ Filets : mv-harnais-achats.mjs (33) + mv-harnais-ateliers.mjs (37, reecrit)
//   + mv-harnais-info.mjs (128) et harnais-claude-md.mjs (27) AJOUTES A LA CHAINE
...
//   inchange), cuve, les deux — et la bascule n'apparait meme pas si le parc
//   a cuves est vide.
//   ⚠️ DEUX PIEGES fermes a l'enregistrement : `nb = total>0 ? total : (_vendDecNb||1)`
//   aurait cree, en mode cuve seule, des BARRIQUES FANTOMES — comptees au parc
//   a futs, comptees dans le volume, porteuses d'un ouillage inexistant. Et
//   tonneaux:[{annee,nb:0}] aurait laisse une entree a zero fut dans la
//   repartition et dans la pyramide des ages.
//   _caveCuveOcc prend un 2e argument `hors` : la cuve qu'on est en train de
//   vider doit rester choisissable, on eleve sur lies dans la cuve de
//   fermentation. Sans lui, l'ecran refusait la seule cuve voulue.
//   Fichiers : cave.js, utils.js, index.html.
