/* ───────────────────────────────────────────────────────────────────────────
   ...
   ★★ LE MODE --contre SUIT LE PATRON DU PROJET (cuv8, recalage) : il n'affiche
   PAS 33 assertions dont 9 en rouge, il dit « detecte » en vert pour chaque
   defaut attrape. Un rouge y signale un defaut qui PASSE.
   ⚠️ Corrige le 13/09, sur remarque de Nico : habituer l'oeil au rouge attendu,
   c'est le rendre aveugle au rouge qui compte.
   ⚠️⚠️ Et les defauts etaient injectes TOUS ENSEMBLE : l'un pouvait masquer
   l'autre. Un par un, chacun doit etre attrape SEUL.
   ─────────────────────────────────────────────────────────────────────────── */
...
function sources(injection) {
  const f = injection || (x => x);
  return { U: f(R('src/utils.js')), CAVE: nu(f(R('src/cave.js'))), PIL: nu(f(R('src/pilotage.js'))),
           REG: nu(f(R('src/reglages.js'))), PHY: nu(f(R('src/phyto.js'))) };
}

/* Les assertions, dans une fonction REJOUABLE : la contre-epreuve la relance
   une fois par defaut injecte, en silence. */
function passe(S, muet) {
  const { U, CAVE, PIL, REG, PHY } = S;
  let ok = 0, ko = 0;
...
if (!contre) {
  console.log('\n\u2500\u2500 AXE-1 \u2014 le cadre de la campagne\n');
  const { ok, ko } = passe(sources(), false);
  console.log('\n  ' + (ko === 0 ? '\x1b[32m' : '\x1b[31m') + ok + ' vert' + (ok > 1 ? 's' : '')
    + ' \u00b7 ' + ko + ' rouge' + (ko > 1 ? 's' : '') + '\x1b[0m\n');
  process.exit(ko === 0 ? 0 : 1);
}

console.log('\n\u2500\u2500 AXE-1 \u2014 contre-epreuve : chaque defaut doit etre attrape SEUL\n');
const refKo = passe(sources(), true).ko;
let rates = 0;
for (const [nom, f] of DEFAUTS) {
  let touche = false;
  const inj = txt => { const b = f(txt); if (b !== txt) touche = true; return b; };
  const { ko } = passe(sources(inj), true);
  ...
  else { rates++; console.log('  \x1b[31mrouge\x1b[0m  ' + nom + '  \u2192 PASSE le harnais'); }
}
console.log('\n  ' + (rates === 0 ? '\x1b[32m' : '\x1b[31m')
  + (DEFAUTS.length - rates) + '/' + DEFAUTS.length + ' defauts attrapes\x1b[0m\n');
