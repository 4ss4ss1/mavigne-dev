...
/* ★★ LES DEFAUTS REINJECTES, AVEC LEUR GARDE.
   ⚠️⚠️ VECU A L'INSTANT : l'injection d'origine visait `if(!_borne) return !!iso;`
...
function injecte(c) {
  return INJ.reduce((a, [nom, f]) => { const b = f(a); if (b !== a) applique.add(nom); return b; }, c);
}
...
if (contre) {
  const morts = INJ.map(d => d[0]).filter(n => !applique.has(n));
  console.log('  ' + (morts.length ? '\x1b[31m✗' : '\x1b[32m✓') + '\x1b[0m '
    + applique.size + '/' + INJ.length + ' defaut(s) reellement injectes'
