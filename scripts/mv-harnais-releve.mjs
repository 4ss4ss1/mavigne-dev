...
console.log('\n2. Le bloc CONTRATS');
T('le bloc est présent', H.indexOf('Contrats \u2014 CDD') !== -1);
T('la première période est datée en toutes lettres',
  H.indexOf('Du 2 mars ' + AN + ' au 24 juillet ' + AN) !== -1);
T('la seconde période aussi, marquée « en cours »',
  H.indexOf('Du 1 septembre ' + AN + ' au 30 novembre ' + AN) !== -1
  && /1 septembre[\s\S]{0,120}cnow/.test(H));
T('la durée de chaque période est comptée', /145 jours/.test(H), 'du 2/03 au 24/07 = 145 j');
T('⚠️ la COUPURE est nommée', H.indexOf('s\u00e9par\u00e9es par une coupure') !== -1);
...
T('deux contrats contigus donnent UNE seule ligne',
  (HC.match(/class="crow"/g) || []).length
    - (HC.match(/Solde initial|Pris sur|<b>Reste<\/b>/g) || []).length === 1,
  'lignes contrat = ' + ((HC.match(/class="crow"/g) || []).length
...
T('mais elle garde son bloc contrats', HV.indexOf('Contrats \u2014') !== -1);
...
if (CONTRE && !ko) {
  ...
  const DEFAUTS = [
    ['le mois du Planning n’est plus restauré',
      ...
      '  planExportPDF(nom);'],
    ['la coupure n’est plus détectée',
      '      if(prev.fin && c.debut) coupure = true;', '      if(false) coupure = true;'],
    ['les contrats ne sont plus datés',
      "      + '<span class=\"cl\">' + (c.debut ? ('Du ' + _plRvDateLg(c.debut)) : 'Depuis toujours')",
      "      + '<span class=\"cl\">' + (c.debut ? 'Sous contrat' : 'Depuis toujours')"],
